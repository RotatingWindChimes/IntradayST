...
from model.single_actor_critic import IntradaySTActorCritic
from trader.daily import daily_test, plot_return, print_action


class SingleACTrainer:
    def __init__(self, time_steps):
        ...
        self.train_env_buffer = DiscreteEnvBuffer(data_type="train", is_train=False)         # 训练环境Buffer
        ...
        self.train_envs, self.train_dates = self.train_env_buffer.sample_all()               # 所有天用于训练
        ...
        self.envs_train, self.dates_train = self.train_env_buffer.sample(date_size=5)

        # reinforce 模型
        self.agent = IntradaySTActorCritic(state_dim=self.train_env_buffer.state_dim(),
        ...
        # 参数路径
        self.params_path = os.path.join("..", "params", "single_ac")
    ...
    def save_model(self, model_path):
        torch.save(self.agent.actor.state_dict(), os.path.join(model_path, "policy_net.params"))
    ...
    def test_model(self, model_path):
        for data_type, dates, envs in [("train", self.dates_train, self.envs_train),
            ...
            for date, env in zip(dates, envs):
                ...
                while not done:
                    if env.tick_index in env.tick_list:
                        # 交易时刻的买卖方向和数量
                        action = self.agent.take_action(state=state)
                    else:
                        action = 50
...
if __name__ == "__main__":
    ...
    trainer = SingleACTrainer(time_steps=options.time_steps)
